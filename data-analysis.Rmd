...
```{r, tidy=TRUE, cache=TRUE}
new.data$comment.time <- new.data$formated.date - hours(new.data$rd.rp.hr) - minutes(new.data$rd.rp.mn)
str(new.data)
summary(new.data)
```
...
```{r, tidy=TRUE, cache=TRUE}
be2olak.data$city <- ifelse(be2olak.data$rd.ri %in% cairo.roads.id, "cairo", "alex")
str(be2olak.data)
