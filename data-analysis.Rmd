...
```{r, tidy=TRUE, cache=TRUE}
be2olak.data <- be2olak.data %>% separate(crawl_date, c("Week_day", "Month", "Month_day", "Crawl_date"), extra = "drop", sep = "[ ]+", convert = T, remove = T) %>% unite(Crawl.time, Week_day, Month, Month_day, Crawl_date, sep = " ", remove = F)
...
be2olak.data$Crawl_date <- NULL
be2olak.data$Crawl_time <- NULL
```
...
Now check if city has introduced NAs.
